...
&& echo "local creation-hash: $(echo ${local} | cast keccak)" \
...
&& if [ "${local}" = "$remote" ]; then \
       ...
       diff <(echo ${local} | fold -w2 | nl -ba) <(echo $remote | fold -w2 | nl -ba) | head -100; \
       exit 1; \
   fi
...
# deployedContractAddress="0xD8EDBb1eeC4dD4778f4642320951ec88beA625F7"
